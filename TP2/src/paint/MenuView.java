	...
	public int getAngle() {
		...
		if ((p.getX() >= center.getX()) && (p.getY() < center.getY()))
			angle = (int) Math.toDegrees(
					Math.acos(distance(center, new Point((int) p.getX(), (int) center.getY())) / distance(center, p)));
		else if ((p.getX() < center.getX()) && (p.getY() < center.getY()))
			angle = 90 + (int) Math.toDegrees(
					Math.asin(distance(center, new Point((int) p.getX(), (int) center.getY())) / distance(center, p)));
		else if ((p.getX() < center.getX()) && (p.getY() >= center.getY()))
			angle = 180 + (int) Math.toDegrees(
					Math.acos(distance(center, new Point((int) p.getX(), (int) center.getY())) / distance(center, p)));
		else
			angle = 270 + (int) Math.toDegrees(
					Math.asin(distance(center, new Point((int) p.getX(), (int) center.getY())) / distance(center, p)));
		return angle;
	}
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < model.currentMenu.length; i++) {
			if (i == model.currentMenu.length - 1) {
				g2.draw(new Arc2D.Double(center.x - RAYON, center.y - RAYON, 2 * RAYON, 2 * RAYON, lastAngle,
						360 - lastAngle, Arc2D.PIE));
				paintText(360, 360 - lastAngle, g2, i);
			} else {
				g2.draw(new Arc2D.Double(center.x - RAYON, center.y - RAYON, 2 * RAYON, 2 * RAYON, lastAngle, angle,
						Arc2D.PIE));
				lastAngle += angle;
